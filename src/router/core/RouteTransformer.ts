...
export class RouteTransformer {
  ...
  private handleFirstLevelRoute(
    ...
    if (route.children && route.children.length > 0) {
      // 有子路由的一级菜单，将原始路由作为子路由，并递归处理其children
      const childRoute: ConvertedRoute = {
        ...route,
        component: component ? this.componentLoader.load(component) : undefined,
        children: route.children.map((child) => this.transform(child, 1))
      }
      converted.children = [childRoute]
    } else {
      // 没有子路由的一级菜单
      converted.children = [
        {
