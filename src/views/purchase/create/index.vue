<!-- 创建采购单 -->
<template>
  <div class="purchase-create-page art-full-height">
    <ElCard shadow="never">
      <template #header>
...
<script setup lang="ts">
  ...
  defineOptions({ name: 'PurchaseCreate' })
  ...
  const handleProductChange = async (productId: number, index: number) => {
    ...
    if (product) {
      ...
      formData.items[index].price = product.cost || 0
      formData.items[index].priceName = '成本价'
      ...
      try {
        ...
        formData.items[index].prices = [
          { price_name: '成本价', price: product.cost || 0 },
          ...prices
        ]
      } catch {
        formData.items[index].prices = [{ price_name: '成本价', price: product.cost || 0 }]
      }
