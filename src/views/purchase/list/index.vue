<!-- 采购订单列表 -->
<template>
  <div class="purchase-list-page art-full-height">
    <!-- 搜索栏 -->
    ...
    <!-- 详情弹窗 -->
    <ElDialog v-model="detailVisible" title="采购单详情" width="800px">
      <div id="printArea">
        ...
        <ElDivider content-position="left">采购明细</ElDivider>
...
<script setup lang="ts">
  ...
  const getStatusType = (status: string): 'warning' | 'primary' | 'success' | 'danger' | 'info' => {
  ...
  const loadData = async () => {
    loading.value = true
    try {
      const result = await fetchGetPurchaseOrderList({
        ...searchForm,
  ...
  const handleApprove = async (row: any) => {
    try {
      await fetchUpdatePurchaseOrderStatus(row.id, 'approved')
  ...
  const handleComplete = async (row: any) => {
    try {
      await fetchUpdatePurchaseOrderStatus(row.id, 'completed')
      ElMessage.success('订单已完成，已自动入库')
      loadData()
    } catch (error) {
  ...
  const handleBatchDelete = async () => {
    try {
      ...
      for (const row of selectedRows.value) {
        await fetchDeletePurchaseOrder(row.id)
      }
  ...
  const handleExport = () => {
    ...
    saveAs(
      ...
      `采购订单_${new Date().toLocaleDateString()}.xlsx`
    )
